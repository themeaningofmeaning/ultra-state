...
from core.map_payload import MapPayloadBuilder
...
class UltraStateApp:
    ...
    def __init__(self):
        ...
        self.map_builder = MapPayloadBuilder(db=self.db)
    ...
    def _build_modal_callbacks(self) -> dict:
        ...
        return {
            ...
            'map_payload_cb':      self.map_builder._get_or_backfill_map_payload,
            'normalize_bounds_cb': self.map_builder._normalize_bounds,
            'calc_distance_cb':    self.map_builder._calculate_distance_from_speed,
            # ── Compute helpers ─────────────────────────────────────────────
    ...
    async def refresh_data_view(self):
        ...
        if self.activities_data:
            if self._map_backfill_task is None or self._map_backfill_task.done():
                self._map_backfill_task = asyncio.create_task(
                    self.map_builder._backfill_map_payloads_for_loaded_activities(
